		...
		if (existingproduct) {
			for (let prod of cart) {
				if (
					...
					quantity <= parseInt(prod.total_allowed_quantity)
				) {
					prod.qty = quantity;
					return true;
				} else if (
...
export const incrementProduct = (product_id, product, quantity) => {
	let cart = JSON.parse(localStorage.getItem("cart"));
	let flag = 0;
	for (let prod of cart) {
		if (prod.product_id === product_id) {
			if (prod.qty + quantity <= parseInt(prod.total_allowed_quantity)) {
				prod.qty += quantity;
				flag = 1;
			} else {
				toast.error("Maximum Quantity Exceeded");
