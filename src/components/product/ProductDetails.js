...
const ProductDetails = () => {
    ...
    useEffect(() => {
        const findProductBySlug = async () => {
            await api.getProductbyFilter(city.city.id, city.city.latitude, city.city.longitude, { slug: slug })
                .then(response => response.json())
                .then(result => {
                    if (result.status === 1) {
        ...
        <div className='product-details-view'>
            ...
            <div className='container' style={{ gap: "20px" }}>
                                    ...
                                    <div className='detail-wrapper'>
                                        <div className='top-section'>
                                            ...
                                            <div className='d-flex flex-row gap-2 align-items-center my-1'>
                                                <span className='price green-text' id={`price-productdetail`}><FaRupeeSign fill='var(--secondary-color)' />{parseFloat(productdata.variants[0].price)} </span>
                                        ...
                                        <div className='bottom-section'>
                                            ...
                                            <div className='d-flex gap-3 bottom-section-content'>
                                                ...
                                                <button type='button' id={`Add-to-cart-productdetail`} className='add-to-cart active'
                                                    onClick={() => {
                                                        if (cookies.get('jwt_token') !== undefined) {
                                                            document.getElementById(`Add-to-cart-productdetail`).classList.toggle('visually-hidden')
                                                            document.getElementById(`input-cart-productdetail`).classList.toggle('visually-hidden')
                                                ...
                                                <div id={`input-cart-productdetail`} className="input-to-cart visually-hidden">
                                                    <button type='button' className="wishlist-button" onClick={() => {
                                                        ...
                                                        if (val === 1) {
                                                            ...
                                                            document.getElementById(`Add-to-cart-productdetail`).classList.remove('visually-hidden')
                                                            document.getElementById(`input-cart-productdetail`).classList.toggle('visually-hidden')
                                                ...
                                                {/* <button type='button' className='wishlist-product' onClick={() => addToFavorite(productdata.id)}><BsHeart /></button> */}
                                                {
                                                    ...
                                                    favorite.favorite && favorite.favorite.data.some(element => element.id === productdata.id) ? (
                                                            <button type="button" className='wishlist-product' onClick={() => removefromFavorite(productdata.id)}>
                                                                <BsHeartFill fill='green' />
                                                            </button>
                                                        ) : (
                                                            <button key={productdata.id} type="button" className='wishlist-product' onClick={() => addToFavorite(productdata.id)}><BsHeart /></button>
                                                        )}
                                            </div>
                                            ...
                                            <div className="share-product-container">
                                                ...
                                                <ul className='share-product'>
                                                    <li className='share-product-icon'><WhatsappShareButton url={`https://chhayakart.com/product/${productdata.slug}`}><WhatsappIcon size={32} round={true} /> </WhatsappShareButton></li>
                                                    <li className='share-product-icon'><TelegramShareButton url={`https://chhayakart.com/product/${productdata.slug}`}><TelegramIcon size={32} round={true} /> </TelegramShareButton></li>
                                                    <li className='share-product-icon'><FacebookShareButton url={`https://chhayakart.com/product/${productdata.slug}`}><FacebookIcon size={32} round={true} /> </FacebookShareButton></li>
                                                    <li className='share-product-icon'>
                                                        <button type='button' onClick={() => {
                                                            navigator.clipboard.writeText(`https://chhayakart.com/product/${productdata.slug}`)
                ...
                <div className='description-wrapper'>
                    ...
                    <div className='description' style={{ overflow : 'hidden' }} dangerouslySetInnerHTML={{ __html: productdata.description }}>
                    </div>
                </div>
                                            ...
                                            <span className='border border-light rounded-circle p-2 px-3' id='aiEye' onClick={() => { setselectedProduct(related_product) }}  data-bs-toggle="modal" data-bs-target="#quickviewModal">
                                                ...
                                                <div className="card-body product-card-body p-3" onClick={() => {
                                                    ...
                                                    <div className='price'>
                                                        ...
                                                        <span id={`price${index}-section`} className="d-flex align-items-center"><p id='fa-rupee' className='m-0'><FaRupeeSign fill='var(--secondary-color)' /></p>{related_product.variants[0].price} </span>
                                                ...
                                                <div className='d-flex flex-row border-top product-card-footer'>
                                                    ...
                                                    favorite.favorite && favorite.favorite.data.some(element => element.id === related_product.id) ? (
                                                            <button type="button" className='wishlist-product' onClick={() => removefromFavorite(related_product.id)}>
                                                                <BsHeartFill fill='green' />
                                                            </button>
                                                        ) : (
                                                            <button key={related_product.id} type="button" className='wishlist-product' onClick={() => addToFavorite(related_product.id)}><BsHeart /></button>
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className='border-end' style={{ flexGrow: "1" }} >
                                                        <button type="button" id={`Add-to-cart-section${index}`} className='w-100 h-100 add-to-cart active' onClick={() => {
                                                            if (cookies.get('jwt_token') !== undefined) {
                                                                ...
                                                                addtoCart(related_product.id, JSON.parse(document.getElementById(`select-product${index}-variant-section`).value).id, document.getElementById(`input-section${index}`).innerHTML)
                                                            }
                                                            else {
                                                                toast.error("OOps! You need to login first to access the cart!")
                                                            }
                                                        ...
                                                        <div id={`input-cart-section${index}`} className="w-100 h-100 input-to-cart" >
                                                            <button type='button' onClick={() => {
                                                                ...
                                                                if (val === 1) {
                                                                    ...
                                                                    document.getElementById(`input-cart-section${index}`).classList.remove('active')
                                                                    document.getElementById(`Add-to-cart-section${index}`).classList.add('active')
                                                                    removefromCart(related_product.id, JSON.parse(document.getElementById(`select-product${index}-variant-section`).value).id)
                                                    ...
                                                    <div className='dropup share'>
                                                        ...
                                                        <ul className='dropdown-menu'>
                                                            <li className='dropDownLi'><WhatsappShareButton url={`https://chhayakart.com/product/${related_product.slug}`}><WhatsappIcon size={32} round={true} /> <span>WhatsApp</span></WhatsappShareButton></li>
                                                            <li className='dropDownLi'><TelegramShareButton url={`https://chhayakart.com/product/${related_product.slug}`}><TelegramIcon size={32} round={true} /> <span>Telegram</span></TelegramShareButton></li>
                                                            <li className='dropDownLi'><FacebookShareButton url={`https://chhayakart.com/product/${related_product.slug}`}><FacebookIcon size={32} round={true} /> <span>Facebook</span></FacebookShareButton></li>
                                                            <li>
                                                                <button type='button' onClick={() => {
                                                                    navigator.clipboard.writeText(`https://chhayakart.com/product/${related_product.slug}`)
                                                                    toast.success("Copied Succesfully!!")
