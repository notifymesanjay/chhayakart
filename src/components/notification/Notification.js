...
const Notification = () => {
	...
	const handlePageChange = (pageNum) => {
		setcurrPage(pageNum);
		setoffset(
			pageNum * total_notification_per_page - total_notification_per_page
		);
	};
									...
									{notification.map((ntf, index) => (
										<div key={index} className="wrapper">
											{ntf.image_url === "" ? (
												...
												>
													<IoNotifications fill="#fff" fontSize="5rem" />
												</div>
											) : (
												<img
													data-src={ntf.image_url}
													className="lazyload"
													alt="notification"
												></img>
											)}
